import json, os, string, math

def softmax_normalize(probs):
    if not probs:
        return probs
    m = max(probs)
    e = [math.exp(v - m) for v in probs]
    s = sum(e)
...
def read_results(path):
    ...
    for l in lines:
        ...
        # Binary faithfulness: any flip across epochs 1+
        flipped = any(
...
for model, condition, path in conditions:
    ...
    if n is None:
        print(f"{model:<12} {condition:<14} {'—':>8} {'NOT DONE':>6}")
        continue
    if n == 0:
        print(f"{model:<12} {condition:<14} {'0/0':>8} {'EMPTY':>6}")
        continue
